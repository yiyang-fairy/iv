$(document).ready(function () {
  ...
  for (let i = 0; i < shopitem.length; i++) {
    ...
    const thisflag = $(shopitem[i]).find(".checkbox").css('"backgroundPositionY"') == "-28px" ? 1 : 0
  ...
  add.click(function () {
    ...
    const originalPrice = Number($(this).parents(".prod-line").find(".price-col").text())
    list[curr][0] = curr
    //价格
  ...
  $(".deleteCurr").click(function () {
    document.querySelectorAll(".shop-cart-wrap").forEach((wrap, index) => {
      wrap.contains(this)
    })
    const wraps = document.querySelectorAll(".shop-cart-wrap")
    let curr = -1
    let fWrap = null
    for (let i = 0; i < wraps.length; i++) {
      const wrap = wraps[i]
      if (wrap.contains(this)) {
        curr = i
        fWrap = wrap
        break
      }
